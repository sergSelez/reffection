      ...
      <div className="container">
        <div className="products_block_wrap">
          <div className="products_head">
            <h2 className="products_head-title section_title white">Продукты Reffection</h2>
          </div>
          <ProductsNewList>
            {products.map((product) => (
              <ProductsNewItem classCol="col-xl-4" key={product.id}>
                ...
                <ProductsNewItemContent>
                  <ProductsNewItemTitle>{product.title}</ProductsNewItemTitle>
                  <div className="products_wrap_item_descr">
                    <CustomText
                      className="white small products_wrap_item-text"
                      dangerouslySetInnerHTML={{ __html: product.description }}
                    ></CustomText>
                  </div>
                </ProductsNewItemContent>
                <Link href={product?.linkPage} aria-label="Подробнее" className="products_wrap_item-btn"></Link>
              </ProductsNewItem>
            ))}
          </ProductsNewList>
        </div>
